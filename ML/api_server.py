...
def parse_budget(budget_str):
    ...
    Handles both en-dash (–) and hyphen (-) as separators,
    with or without commas (e.g., 'EGP 21000 – EGP 82000').
    """
    try:
        # Normalize: remove EGP, commas, then replace en-dash with hyphen
        clean = (
            budget_str
            .replace("EGP", "")
            .replace(",", "")
...
def calculate_realistic_budget(budget_min, budget_max, hotels, restaurants, places, start_date, end_date, group_size, acc_type, transportation):
    ...
    # ── Clamp total to the user's selected budget range ─────────────────────
    # DB prices are often stale / low defaults; always honour the user's choice.
    if budget_min > 0 and real_total < budget_min:
        scale = budget_min / real_total if real_total > 0 else 1
        accommodation_cost = round(accommodation_cost * scale)
        food_cost          = round(food_cost          * scale)
        activities_cost    = round(activities_cost    * scale)
        transport_cost     = round(transport_cost     * scale)
        real_total         = budget_min
    elif budget_max > 0 and real_total > budget_max:
        scale = budget_max / real_total if real_total > 0 else 1
        accommodation_cost = round(accommodation_cost * scale)
        food_cost          = round(food_cost          * scale)
        activities_cost    = round(activities_cost    * scale)
        transport_cost     = round(transport_cost     * scale)
        real_total         = budget_max
...
def generate_trip():
    print("\n🚨🚨🚨 DOORBELL RINGING! PYTHON WAS HIT! 🚨🚨🚨", flush=True)
    try:
        data = request.json
        ...
        # ── Try to parse the LLM output as our exact JSON schema ──────────────
        llm_json = None
        raw = trip_plan_text.strip()
        ...
        try:
            ...
            print("[API] ✅ LLM returned valid JSON — using it directly.", flush=True)
        except (json.JSONDecodeError, ValueError):
            print("[API] ⚠️  LLM output is not JSON — falling back to Markdown parser.", flush=True)
            llm_json = None
        ...
        if llm_json and "activities" in llm_json:
            ...
            response_payload = {
                "hotelName":    llm_json.get("hotelName", ""),
                "hotelDetails": llm_json.get("hotelDetails", ""),
                "activities":   llm_json.get("activities", []),
                # Include enriched places and restaurants
                "places":       enriched_places,
                "restaurants":  enriched_restaurants,
                "overview":     llm_json.get("overview", ""),
                "hotels":       llm_json.get("hotels", []),
                "tips":         llm_json.get("tips", []),
                "content":      raw,   # raw JSON stored as content so C# can also read it
                "budget":       calculate_realistic_budget(
            ...
            if not response_payload["overview"]:
                response_payload["overview"] = summary
            return jsonify(response_payload)

        # ── Fallback: LLM returned Markdown — use the legacy regex parser ──────
