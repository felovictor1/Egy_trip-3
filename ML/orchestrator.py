...
class TripOrchestrator:
    ...
    def generate_trip_plan(
        ...
        try:
            ...
            # result might be a string or an object with .content
            content_to_print = result.content if hasattr(result, 'content') else result
            print(f"====== RAW AI RESPONSE ======\n{content_to_print}", flush=True)
            
        except Exception as e:
            print(f"====== CRITICAL LLM ERROR ======\n{str(e)}", flush=True)
            raise e # Crash the server to force the red text!
